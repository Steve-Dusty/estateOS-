...
const intentBadge: Record<string, { label: string; color: string }> = {
  navigate:   { label: 'NAVIGATE',   color: 'var(--accent)' },
...
const pageRoutes: Record<string, string> = {
  ...
  portfolio:    '/portfolio',
  client:       '/client',
  'world builder': '/client',
  world:        '/client',
};

const toolLabel: Record<string, { label: string; color: string }> = {
  navigate:   { label: 'NAV',   color: 'var(--accent)' },
...
export default function VoiceAssistant() {
  ...
  const handleRecordingStop = useCallback(async () => {
    ...
    try {
      ...
      // Handle navigate intent — skip tool execution, go straight to page
      if (intent === 'navigate') {
        const pageName = (data.page || data.subject || '').toLowerCase().trim();
        const route = pageRoutes[pageName];
        if (route) {
          addAssistantMessage({ intent, message: `Navigating to ${pageName}…` });
          const navId = addLog('NAV', `→ ${route}`, 'var(--accent)');
          await new Promise(r => setTimeout(r, 800));
          resolveLog(navId, `→ ${route}`);
          window.location.href = route;
        } else {
          addAssistantMessage({ intent, message: `I don't know the page "${pageName}". Try: overview, intelligence, reports, portfolio, or client.` });
        }
      } else {
        ...
        // Navigate to the relevant page for non-navigate intents
